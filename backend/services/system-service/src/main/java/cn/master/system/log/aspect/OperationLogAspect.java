package cn.master.system.log.aspect;
...
public class OperationLogAspect {
    ...
    @Pointcut("@annotation(cn.master.system.log.annotation.Log)")
    ...
    public void before(JoinPoint joinPoint) {
        try {
            localUser.set(SecurityUtils.getUserId());
            localOrganizationId.set(SecurityUtils.getCurrentOrganizationId());
            localProjectId.set(SecurityUtils.getCurrentProjectId());
            MethodSignature signature = (MethodSignature) joinPoint.getSignature();
            Method method = signature.getMethod();
            Log msLog = method.getAnnotation(Log.class);
            if (Objects.nonNull(msLog) && isMatch(msLog.type())) {
                // 获取参数对象数组
    ...
    private String getId(Object result) {
        try {
            if (result != null) {
                ...
                Map object = JsonUtils.objectToType(new TypeReference<Map>() {
                }).apply(resultStr);
    ...
    private boolean isMatch(OperationLogType keyword) {
        return Arrays.stream(beforeMethodNames).anyMatch(input -> input.contains(keyword));
    }
